        ...
        <div class={"flex flex-row justify-center gap-2 mx-auto mb-4"}>
          {podium.map((team, i) => (
            <div class={` flex flex-col gap-1 justify-end items-center `}>
              <img
                style={{ height: 40 }}
                src={`https://api.promiedos.com.ar/images/team/${team.team.id}/1`}
              />
              <div
                class={`flex items-center justify-center border-t border-[#c2e213] bg-[#071811] ${team.pos === 1 && "h-[60px]"} ${team.pos === 2 && "h-[40px]"} ${team.pos === 3 && "h-[23px]"} w-[80px] text-2xl font-semibold`}
              >
                {team.pos}
              </div>
            </div>
...
const Stage = ({ stage }) => (
  <div class={"flex flex-col justify-center md:w-auto w-[200px]"}>
    <div
      class={
        "z-80 sticky top-0 w-full shadow-xs shadow-black bg-[#004128]  text-xs font-semibold text-center py-2"
      }
    >
    ...
    <div
      class={`relative flex flex-col ${stage.name === "Final" && stage.groups.length > 1 ? "justify-between" : "justify-around"} gap-[2px] pt-2  w-full h-full  bg-[#0f2217]/50   `}
    >
      {stage.name === "Final" && stage.groups.length == 2 && (
        <div class={"mb-20"}></div>
      )}
      ...
      {stage.groups.map((match, index) => {
        return (
          <div
            class={`${index % 2 != 0 && index != stage.groups.length - 1 ? "mb-6" : ""} `}
          >
            {stage.name === "Final" && index === 1 && (
              <div
                class={
                  "bg-[#004128] w-full py-0 text-center font-semibold text-[11px] mb-1"
                }
              >
                TERCER PUESTO
              </div>
            ...
            <Match key={match.participants[0].id || index} match={match} />
          </div>
        );
      })}
    </div>
...
const Match = ({ match }) => {
  ...
  return (
    ...
    >
      <div
        class={
          "bg-[#065a1f] flex flex-row items-center gap-1 h-[14px] justify-center text-center text-[10px] text-white"
        }
      >
      ...
      <div
        class={`flex w-full  flex-row justify-between  transition-all  relative my-0 pl-1  overflow-hidden  `}
      >
        ...
        <div class={"flex gap-0"}>
          {"score" in match && (
            <div
              class={
                "pr-1 flex flex-col items-center justify-center divide-y-[1px] border-[#008000]/50 "
              }
            >
              {match.score.map((item) => (
                <div
                  class={`py-0 border-l-[0px] border-[#008000]/50 border-r-[0px] flex-1  font-bold`}
                >
                  {item}
                </div>
              ))}
          ...
          {match.games?.length > 1 && (
            <div
              class={`flex flex-col items-center justify-start gap-0  ${match.games?.length > 1 && "border-l-[1px] border-[#008000]/50"}`}
            >
              {match.games.map((game, i) => {
                return (
                  "scores" in game && (
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          class={
                            "flex flex-row items-center h-full justify-center "
                          }
                        >
...
const Team = ({ name, id, winner }) => (
  <div class="flex flex-row flex-1 justify-start gap-1 items-center w-full py-1">
    ...
    <span
      class={` ${winner ? "font-semibold" : ""} line-clamp-1 text-xs py-0 pr-[1px]`}
      title={name}
    >
      {name}{" "}
    </span>
...
const getTeams = (stages) => {
  ...
  if (team_1 === undefined || team_2 === undefined || team_1 === undefined)
    return undefined;
