...
import CircularBrackets from "@/components/common/CircularBrackets";
...
const League = ({ id }) => {
  ...
  useEffect(() => {
    ...
    if (query.tab != undefined) selectedTab.value = query.tab;
    else selectedTab.value = window.innerWidth < 768 ? "fixture" : "principal";
  }, [league]);
  ...
  return (
    <div class="relative min-h-screen md:mx-12 md:rounded-t md:p-0   md:mt-2  overflow-x-auto px-0 shadow-black md:shadow-lg bg-background/70 border-x-[1px] border-borderc  md:col-start-2 flex flex-col">
      ...
      <div class={"md:order-2 mx-1 mb-20"}>
        <div
          class={`${selectedTab.value === "principal" ? "" : "hidden"}  grid ${league.tables_groups != undefined ? "grid-cols-2 mx-4" : "grid-cols-1 mx-50"} gap-10 md:mt-7  mb-20`}
        >
          <div class={"flex flex-col gap-5"}>
            {id === "fjda" && <CircularBrackets id={"fjda"} />}
        ...
        <div
          class={`${selectedTab.value === "llaves" ? "" : "hidden"} overflow-x-auto mt-3`}
        >
          <Brackets id={id} show={league.brackets != undefined} />
        </div>

        <div
          class={`${selectedTab.value === "estadisticas" ? "" : "hidden"} overflow-x-auto mt-3`}
        >
          <Stats id={id} />
        </div>
      </div>
