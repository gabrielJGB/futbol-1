  ...
  return (
    <div
      ...
      class={`z-100  ${showMenu.value ? "md:block fixed -left-[0%]  backdrop-blur-xs" : "md:block md:-left-[0%]  backdrop-blur-none fixed  -left-[100%]"} 
         bottom-0  flex flex-col gap-0 h-[93vh] md:w-[250px] w-full shadow-lg shadow-black   overflow-y-auto transition-all`}
    >
            ...
            <div class={"flex flex-col gap-[0px]  "}>
              {
                section.items.map((item, j) => (
                  <Link href={`/league/${item.id}`} class={` odd:bg-b2 even:bg-b3 transition-all text-sm text-white text-shadow-xs text-shadow-black px-2 md:py-1 py-3  active:bg-[#077227] hover:bg-[#077227]`}>
                    {item.league}</Link>
                ))
              }
            </div>
