  ...
  return (
    <div class=" rounded-lg shadow-lg mb-8">
      ...
      <p class="text-sm text-gray-400  mb-2">
        Mostrando estadísticas de {selectedLeague.name}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 ">
        {selectedLeague.tables.map(table => (
          <div key={table.name} class=" shadow-xs shadow-black bg-gray-800  not-last:border-gray-500 md:p-3 p-2  rounded-lg">
            <h4 class="font-semibold text-lg text-blue-400 mb-2 border-l-4 pl-2 border-blue-700">{table.name}</h4>
            <ul class="flex flex-col gap-[2px] bg-gray-500 p-[2px]">
              {table.rows.slice(0, 6).map(row => ( // Mostramos solo los primeros 6 para brevedad
                <li key={row.entity.object.name} class=" even:bg-[#E7E7E7] odd:bg-[#D5D5D5] text-black hover:bg-gray-400 cursor-pointer border-gray-800 hover flex justify-between items-center py-[2px] not-last:border-b-[0px] px-2">
