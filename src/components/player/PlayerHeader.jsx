...
const PlayerHeader = ({ name }) => {
  ...
  return (
    <div
      class={
        "grid md:grid-cols-2 grid-cols-1 md:gap-0 gap-0 md:m-0 m-0 text-white  border-white/10  overflow-hidden "
      }
    >
      <div
        class={
          "relative flex gap-2 md:items-start items-center md:p-3 px-3 py-1 bg-[#0e3e1d]"
        }
      >
        ...
        <div>
          <div
            class={
              "text-[#C2E213] font-bold md:text-[27px] text-[25px] text-shadow-xs text-shadow-black leading-tight"
            }
          >
            {player.name}
          </div>
              ...
              <div class={"flex flex-col "}>
                <div class={"flex flex-row items-center gap-1"}>
                  ...
                  <div class={"text-red-300 text-xs"}>
                    {player.injury.name} (
                    {new Date(player.injury.startDate).toLocaleDateString()})
                  </div>
                </div>
                <div class={"text-red-300  text-[11px]"}>
                  Vuelta: {translateString(player.injury.expectedReturn)}
                </div>
              </div>
            </div>
          )}
      ...
      <div class={"px-3 bg-[#0e3e1d]"}>
        <div
          class={
            " grid grid-cols-2 gap-px  md:border-y-0 border-y border-white/10"
          }
        >
          ...
          <div
            class={
              "bg-[#0e3e1d] p-2 flex items-center gap-2 border-l border-white/10"
            }
          >
            ...
            <div class={" leading-tight"}>
              ...
              {"contractUntil" in player && (
                <div class={"text-[10px] text-gray-400"}>
                  Fin contrato:{" "}
                  {new Date(player.contractUntil).toLocaleDateString()}
                </div>
              )}
            </div>
          </div>
        ...
        <div class={"bg-[#0e3e1d] grid md:grid-cols-3  grid-cols-2 gap-x-5"}>
          {player.playerDetails.map((detail) => (
            <div
              class={
                "flex flex-row md:flex-col justify-between items-center border-t border-white/5 py-1"
              }
            >
              <div class={"text-[10px] uppercase text-gray-400"}>
                {detail.title}
              </div>
              <div class={"text-xs font-bold text-[#C2E213]"}>
...
const getImgElement = (id, type, size = 30) => {
  const urls = {
    ...
    player:
      "https://imagecache.365scores.com/image/upload/f_png,w_150,h_150,c_limit,q_auto:eco,dpr_2,d_Athletes:default.png,r_max,c_thumb,g_face,z_0.65/Athletes/",
