...
const Team = ({ name, id, winner }) => (
  <div class="flex flex-row flex-1 justify-start gap-1 items-center w-full">
    ...
    <span class={` ${winner ? "font-semibold" : ""} text-xs py-2`}>
      {name}{" "}
    </span>
  </div>
);
    ...
    >
      ...
      <div class={"flex align-middle gap-1"}>
        {"score" in match && (
          <div
            class={
              "flex-1 flex flex-col items-center justify-center divide-y-[1px] border-[#008000]/50 "
            }
          >
            {match.score.map((item) => (
              <div
                class={`pt-1 pr-1 border-l-[0px] border-[#008000]/50 border-r-[0px] flex-1 font-bold`}
              >
                {item}
              </div>
            ))}
        ...
        {match.games?.length > 1 && (
          <div
            class={`flex flex-col items-center justify-between gap-0 pl-1  ${match.games?.length > 1 && "border-l-[1px] border-[#008000]/50"}`}
          >
              ...
              return (
                "scores" in game && (
                  ...
                  >
                    ...
                    <div
                      class={"flex flex-row items-center h-max justify-center "}
                    >
...
const Stage = ({ stage }) => (
  <div
    class={`relative flex flex-col ${stage.name === "Final" && stage.groups.length > 1 ? "justify-between" : "justify-around"} gap-[2px] md:w-[230px] w-[200px]  h-full  bg-[#0f271a] p-1 pt-2 rounded-b`}
  >
      ...
      return (
        ...
        >
          {stage.name === "Final" && index === 1 && (
            <div
              class={
                "bg-[#004128] w-full py-1 text-center font-semibold text-[11px] mb-1"
              }
            >
              TERCER PUESTO
            </div>
  ...
  return (
    <>
      ...
      <div class={"flex flex-col justify-center mx-auto  overflow-auto"}>
        ...
        <div
          class={"grid sticky top-0 w-min z-30 gap-1 "}
          style={{ gridTemplateColumns: `repeat(${cols},1fr)` }}
        >
          {data.stages.map((stage) => (
            <div
              class={
                "md:w-[230px] w-[200px] bg-[#004128]/85  text-xs font-semibold text-center py-2"
              }
            >
        ...
        <div
          style={{ gridTemplateColumns: `repeat(${cols},1fr)` }}
          class=" relative  grid gap-1 h-min w-min"
        >
          {data.stages.map((stage) => (
