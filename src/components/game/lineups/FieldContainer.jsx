...
const FieldContainer = ({ id }) => {
  ...
  const assists =
    data.game.events != undefined &&
    data.game.events
      .flatMap((x) => x.rows)
      .filter((e) => e.events[0].type === 1 && e.events[0].texts.length > 1)
      .map((e) => e.events[0].texts[1]);
  ...
  return (
    <div class={"flex flex-col gap-1"}>
      ...
      <div
        class={`${darkMode.value ? "bg-[url('/fieldbg_dark.png')]" : "bg-[url('/fieldbg.png')]"} z-0 grid grid-cols-2 bg-repeat shadow-gray-950 md:h-[450px] h-[430px] relative rounded-lg  md:w-full w-[200vw]`}
      >
        <FieldLinesDrawing />
        <FormationStrings formations={formations} ids={ids} />

        {teams.map((team, i) => (
          <TeamField
            isHome={i == 0}
            team={{ ...team, colors: colors[i] }}
            gameId={id}
            teamId={ids[i]}
            assists={assists}
          />
