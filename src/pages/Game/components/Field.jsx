...
const Field = ({ teams, colors, ids }) => {
    ...
    const getPlayerElement = (player, i) => {
        ...
        return (
            <div class={" flex flex-row gap-1 items-center justify-center  text-white text-xs font-bold z-10 "}>


                <div
                    style={{ backgroundColor: colors[i].color, color: colors[i].text_color }}
                    class={"flex border-[2px] border-gray-900 justify-center items-center rounded-lg text-[10px] md:text-[12px] font-bold px-[2px] py-[0px] w-[20px] h-min  shadow-xs shadow-gray-800"}>{player.jersey_num}</div>
                <div
                    style={{ textShadow: "black 1px 1px 3px", }}
                    class={`${getPlayerNameColor(player)} flex flex-row items-center gap-1 max-w-[100px] text-center text-[12px]  px-[0px]`}>{player.player_short_name}{player.is_captain ? " (C)" : ""}
    ...
    const homeBench = teams[0].bench
    const awayBench = teams[1].bench
    ...
    return (
        <div class={" w-full "}>
            <div class={" bg-[url('/fieldbg.png')] bg-repeat md:w-full w-[200vw]  shadow-xs shadow-gray-950   md:h-[490px] h-[520px] relative rounded-lg"}>
            ...
            > */}
                ...
                {home.map((p, i) => {
                    ...
                    const leftPercent = clamp((x) * 0.45, 4, 45); // 0..100 -> 0..50 -> limitado a 5..45
                    const topPercent = clamp((y + 2) * 0.9, 10, 98);
                    return (
                        <div
                            key={`local-${p.jersey_num}-${i}`}
                            className="absolute flex  flex-col gap-[1px] items-center justify-center  text-white text-xs font-bold z-10  "
                            style={{
                                left: `${leftPercent}%`,
                                top: `${topPercent / 0.99}%`,
                        ...
                        >
                            ...
                            <div
                                style={{ textShadow: "black 1px 1px 3px", }}
                                class={`${getPlayerNameColor(p, "substitution" in p)} flex flex-row items-center gap-1 text-center text-[14px]  px-[2px]`}>{p.player_short_name}{p.is_captain ? " (C)" : ""}
...

                                {
                                    ...
                                    <div class={"flex flex-row items-center gap-[1px]"}>
                                        {
                                            Array.from({ length: p.events.goals.goals }).map((_, i) => (
...

                            {
                                ...
                                getPlayerElement(homeBench.find(item => item.jersey_num === p.substitution.player), 0)
                ...
                {away.map((p, i) => {
                    ...
                    const leftPercent = 50 + clamp((invertedX + 3) * 0.50, 5, 46); // 50 + (0..50) limitado -> 55..95
                    const topPercent = clamp((invertedY), 10, 90);
                    return (
                        <div
                            key={`away-${p.jersey_num}-${i}`}
                            className="absolute flex flex-col gap-1 items-center justify-center  text-white text-xs font-bold z-10 max-w-[82px]"
                            style={{
                                left: `${leftPercent}%`,
                                top: `${topPercent}%`,
                                transform: "translate(-50%, -50%)",
                        ...
                        >
                            ...
                            >
                                ...
                                <span class={`text-shadow-xs z-20 ${ids[0] === "igg" || ids[1] === "igg" ? "text-black" : ""}`}>{p.jersey_num}</span>

                            </div>
                            <div
                                style={{ textShadow: "black 1px 1px 3px", }}
                                class={`${getPlayerNameColor(p)} text-center text-[14px]`}>{p.player_short_name}{p.is_captain ? " (C)" : ""}
                            </div>

                            {
                                "substitution" in p &&

                                getPlayerElement(awayBench.find(item => item.jersey_num === p.substitution.player), 1)
